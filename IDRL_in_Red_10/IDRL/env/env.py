...
from IDRL.env.game import GameEnv
...
class Env:
    ...
    def reset(self):
        ...
        card_play_data = {'landlord': _deck[:13],
                          'landlord_up': _deck[13:26],
                          'landlord_down': _deck[26:39],
                          'landlord_front':_deck[39:52],
    ...
    def _get_reward(self):
        ...
        """
        winner = self._game_winner  #update_num_wins_scores 中赋值，值为landlord or farmer
        if winner == 'landlord':
            return 1.0
        else:
            return -1.0
...
def _cards2array_R_D(cards):
    ...
    for card, num_times in counter.items():
        if card == 'B':
            ...
            matrix[:, Card2Column_R_D[card]] = NumOnes2Array[num_times]
    return np.concatenate((matrix.flatten('F'), jokers))
...
def _get_obs_landlord(infoset):
    ...
    last_landlord_up_action = _cards2array(
        infoset.last_move_dict['landlord_up'])
    last_landlord_up_action_batch = np.repeat(
        last_landlord_up_action[np.newaxis, :],
        num_legal_actions, axis=0)
    landlord_up_num_cards_left = _get_one_hot_array(
        infoset.num_cards_left_dict['landlord_up'], 13)
    ...
    last_landlord_front_action = _cards2array(
        infoset.last_move_dict['landlord_front'])
    last_landlord_front_action_batch = np.repeat(
        last_landlord_front_action[np.newaxis, :],
        num_legal_actions, axis=0)
    landlord_front_num_cards_left = _get_one_hot_array(
        infoset.num_cards_left_dict['landlord_front'], 13)
    ...
    last_landlord_down_action = _cards2array(
        infoset.last_move_dict['landlord_down'])
    last_landlord_down_action_batch = np.repeat(
        last_landlord_down_action[np.newaxis, :],
        num_legal_actions, axis=0)
    landlord_down_num_cards_left = _get_one_hot_array(
        infoset.num_cards_left_dict['landlord_down'], 13)
    ...
    x_batch = np.hstack((my_handcards_batch,
                         ...
                         last_action_batch,
                         last_landlord_up_action_batch,
                         last_landlord_front_action_batch,
                         last_landlord_down_action_batch,
    ...
    x_no_action = np.hstack((my_handcards,
                             ...
                             last_action,
                             last_landlord_up_action,
                             last_landlord_front_action,
                             last_landlord_down_action,
...
def _get_obs_landlord_up(infoset):
    ...
    landlord_num_cards_left = _get_one_hot_array(
        infoset.num_cards_left_dict['landlord'], 13)
