...
class MemoryGroup(object):
    ...
    def push(self, **kwargs):
        for i, _id in enumerate(kwargs['ids']):
            ...
            if self.use_mean:
                self.agent[_id].append(obs0=kwargs['state'][0], feat0=kwargs['state'][1], act=kwargs['acts'], reward=kwargs['rewards'], alive = kwargs['alives'], prob=kwargs['prob'])
            else:
                self.agent[_id].append(obs0=kwargs['state'][0], feat0=kwargs['state'][1], act=kwargs['acts'], reward=kwargs['rewards'], alive = kwargs['alives'])
...
class Runner(object):
    ...
    def run(self, variant_eps, iteration, win_cnt=None):
        ...
        _,_,winner_p = self.play(env=self.env, n_round=iteration, map_size=self.map_size, max_steps=self.max_steps, handles=self.handles,
        ...
        if self.train:
            ...
            if self.save_every and (iteration + 1) % self.save_every == 0:
                ...
                self.models.save(self.model_dir , iteration)
