...
class ValueNet:
    def __init__(self, sess, env, handle, name, update_every=5, use_mf=False, learning_rate=1e-4, tau=0.005, gamma=0.95):
        ...
        self.handle = handle
        _,_,_,red10_state = env.reset()
        x_shape =red10_state['x_batch'].shape
        x_batch = np.reshape(red10_state['x_batch'],(x_shape[0],11,54))
        self.view_space = (1,11,54)
        assert len(self.view_space) == 3
        self.feature_space = (54,)
        self.num_actions = 1
        ...
        with tf.variable_scope(name or "ValueNet"):
            ...
            with tf.variable_scope("Eval-Net"):
                ...
                self.predict = (self.e_q / self.temperature)
    ...
    def _construct_net(self, active_func=None, reuse=False):
        conv1 = tf.layers.conv2d(self.obs_input, filters=32, kernel_size=3,padding='same',
                                 activation=active_func, name="Conv1")
        conv2 = tf.layers.conv2d(conv1, filters=32, kernel_size=3, activation=active_func,padding='same',
                                 name="Conv2")
    ...
    def act(self, **kwargs):
        ...
        if self.use_mf:
            assert kwargs.get('prob', None) is not None
            assert len(kwargs['prob']) == (kwargs['state'][0].shape[0])
            feed_dict[self.act_prob_input] = kwargs['prob']

        actions = self.sess.run(self.predict, feed_dict=feed_dict)
        actions = np.argmax(actions, axis=0).astype(np.int32)
        return actions
