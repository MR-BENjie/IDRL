...
class Red_10RuleModelV1(Model):
    ...
    def agents(self,rela_state=None,danger_state=None,rule_id=0):
        ''' Get a list of agents for each position in a the game
...
class Red_10RuleModelV2(Model):
    ...
    def agents(self,rela_state=None,danger_state=None,role_id=0):
        ...
        if rela_state and danger_state:
            return 0.0,0.0,1000,agent[role_id]
        else:
            return agent
...
class RHCPRuleAgent(object):
    ...
    def step(self, state):
        ...
        # the rule of following cards
            ...
            target = state['trace'][-1][-2]
            target_player = state['trace'][-1][0]
            if target == 'pass':
                target = state['trace'][-2][-2]
                target_player = state['trace'][-1][0]
            if target == 'pass':
                target = state['trace'][-3][-2]
                target_player = state['trace'][-2][0]
            if target == 'pass':
                target = state['trace'][-4][-2]
                target_player = state['trace'][-3][0]
            the_type = CARD_TYPE[0][target][0][0]
            chosen_action = ''
            rank = 1000
    ...
    def eval_step(self, state):
        ''' Step for evaluation. The same to step
        '''
        choice_action = self.step(state=state)
        info = {}
        info['probs'] = {}
        info['probs'][choice_action] = 1
        return choice_action, info
...
class RHCPRuleModel(Model):
    ...
    def agents(self,rela_state=None,danger_state=None,role_id=0):
        ...
        '''
        if rela_state and danger_state:
            return 1, 1, 1000, self.rule_agents[0]
        else:
            return self.rule_agents
...
class RandomAgent(object):
    ...
    def eval_step(self, state):
        ...
        Returns:
            ...
            probs (list): The list of action probabilities
        '''
        probs = [0 for _ in range(len(state['legal_actions']))]
        for i in range(len(state['legal_actions'])):
            probs[i] = 1/len(state['legal_actions'])

        info = {}
        info['probs'] = {state['raw_legal_actions'][i]: probs[i] for i in range(len(state['legal_actions']))}
